 ...
 */
@Setter
public class Cn0002CsvBuilder extends AbstractMakeCsvBuilder<CsvBuilderDto> {
	...
	public CsvRowDto getHeaderRowDto() {
		...
		itemList.add(new CsvRowItemDto("属性"));
		...
		itemList.add(new CsvRowItemDto("案件"));
	...
	public void makeCsvFile(HttpServletResponse response) throws IOException, Exception {
		super.makeCsvFile(OutputCsvTemplate.CSV_NAIBU_2, this.csvBuilderDto, response);
	}
