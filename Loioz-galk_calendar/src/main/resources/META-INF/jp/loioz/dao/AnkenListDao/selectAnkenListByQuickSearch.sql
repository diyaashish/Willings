...
WHERE
  -- ステータスが該当する条件を追加
/*%if !ankenStatusList.isEmpty() */
  tac.anken_status IN /*ankenStatusList*/(1) 
/*%end*/
/*%if ankenStatusList.isEmpty() */
  tac.anken_status IS NULL 
/*%end*/
  -- 一覧のキーワード検索と同じ条件
  AND EXISTS (
    SELECT
    ...
    FROM 
      ( 
        ...
        WHERE
            -- 案件ID
            ta.anken_id LIKE /* @infix(quickSearchText) */NULL

            -- 案件名検索
            OR REPLACE (ta.anken_name, '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (ta.anken_name, ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            
            -- 関与者名検索 
            OR REPLACE (CONCAT(COALESCE(tkc.kanyosha_name_sei, ''), COALESCE(tkc.kanyosha_name_mei, '')), '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (CONCAT(COALESCE(tkc.kanyosha_name_sei, ''), COALESCE(tkc.kanyosha_name_mei, '')), ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
             
            -- 関与者名（かな）検索
            OR REPLACE (CONCAT(COALESCE(tkc.kanyosha_name_sei_kana, ''), COALESCE(tkc.kanyosha_name_mei_kana, '')), '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (CONCAT(COALESCE(tkc.kanyosha_name_sei_kana, ''), COALESCE(tkc.kanyosha_name_mei_kana, '')), ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 関与者旧姓検索
            OR REPLACE (CONCAT(COALESCE(tkc.kanyosha_old_name, ''), COALESCE(tkc.kanyosha_name_mei, '')), '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (CONCAT(COALESCE(tkc.kanyosha_old_name, ''), COALESCE(tkc.kanyosha_name_mei, '')), ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 関与者旧姓（かな）検索
            OR REPLACE (CONCAT(COALESCE(tkc.kanyosha_old_name_kana, ''), COALESCE(tkc.kanyosha_name_mei_kana, '')), '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (CONCAT(COALESCE(tkc.kanyosha_old_name_kana, ''), COALESCE(tkc.kanyosha_name_mei_kana, '')), ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 関与者代表者検索
            OR REPLACE (tkc.daihyo_name, '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (tkc.daihyo_name, ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 関与者代表者(かな)検索
            OR REPLACE (tkc.daihyo_name_kana, '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (tkc.daihyo_name_kana, ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 関与者担当者検索
            OR REPLACE (tkc.tanto_name, '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (tkc.tanto_name, ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 関与者担当者(かな)検索
            OR REPLACE (tkc.tanto_name_kana, '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (tkc.tanto_name_kana, ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 関与者屋号検索
            OR REPLACE (COALESCE(tkc.kanyosha_yago, ''), '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (COALESCE(tkc.kanyosha_yago, ''), ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 関与者屋号（かな）検索
            OR REPLACE (COALESCE(tkc.kanyosha_yago_kana, ''), '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (COALESCE(tkc.kanyosha_yago_kana, ''), ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 顧客名検索
            OR REPLACE (CONCAT(COALESCE(tc.customer_name_sei, ''), COALESCE(tc.customer_name_mei, '')), '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (CONCAT(COALESCE(tc.customer_name_sei, ''), COALESCE(tc.customer_name_mei, '')), ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 顧客名検索（かな）
            OR REPLACE (CONCAT(COALESCE(tc.customer_name_sei_kana, ''), COALESCE(tc.customer_name_mei_kana, '')), '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (CONCAT(COALESCE(tc.customer_name_sei_kana, ''), COALESCE(tc.customer_name_mei_kana, '')), ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 顧客名旧姓検索 
            OR REPLACE (CONCAT(COALESCE(tcak.old_name, ''), COALESCE(tc.customer_name_mei, '')), '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (CONCAT(COALESCE(tcak.old_name, ''), COALESCE(tc.customer_name_mei, '')), ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 顧客名旧姓（かな）検索
            OR REPLACE (CONCAT(COALESCE(tcak.old_name_kana, ''), COALESCE(tc.customer_name_mei_kana, '')), '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (CONCAT(COALESCE(tcak.old_name_kana, ''), COALESCE(tc.customer_name_mei_kana, '')), ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 顧客名代表者検索
            OR REPLACE (tcah.daihyo_name, '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (tcah.daihyo_name, ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 顧客名代表者(かな)検索
            OR REPLACE (tcah.daihyo_name_kana, '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (tcah.daihyo_name_kana, ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 顧客名担当者検索
            OR REPLACE (tcah.tanto_name, '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (tcah.tanto_name, ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 顧客名担当者(かな)検索
            OR REPLACE (tcah.tanto_name_kana, '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (tcah.tanto_name_kana, ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 顧客屋号検索
            OR REPLACE (COALESCE(tcak.yago, ''), '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (COALESCE(tcak.yago, ''), ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL

            -- 顧客屋号（かな）検索
            OR REPLACE (COALESCE(tcak.yago_kana, ''), '　', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
            OR REPLACE (COALESCE(tcak.yago_kana, ''), ' ', '') COLLATE utf8mb4_unicode_ci LIKE /* @infix(quickSearchText) */NULL
      ) AS exists_t
    WHERE
